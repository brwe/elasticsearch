...
public class ExceptionRetryIT extends ESIntegTestCase {
    ...
    @Test
    public void testRetryUpsertDueToExceptionOnNetworkLayer() throws ExecutionException, InterruptedException, IOException {
        final AtomicBoolean exceptionThrown = new AtomicBoolean(false);
        int numDocs = scaledRandomIntBetween(100, 1000);
        NodesStatsResponse nodeStats = client().admin().cluster().prepareNodesStats().get();
        NodeStats unluckyNode = randomFrom(nodeStats.getNodes());
        ...
        for (int i = 0; i < numDocs; i++) {
            XContentBuilder doc = null;
            doc = jsonBuilder().startObject().field("foo", "bar").endObject();
            bulkBuilder.add(client().prepareUpdate().setIndex("index").setType("type").setId(Integer.toString(i)).setDoc(doc)
                    .setUpsert((new IndexRequest("index", "type", Integer.toString(i))).source("foo", "bar")));
        }
        ...
        if (response.hasFailures()) {
            for (BulkItemResponse singleIndexRespons : response.getItems()) {
                if (singleIndexRespons.isFailed()) {
                    if (singleIndexRespons.getFailure().getCause() instanceof DocumentAlreadyExistsException == false)
                    fail("None of the bulk items should fail but got " + singleIndexRespons.getFailureMessage());
                }
            }
        }
