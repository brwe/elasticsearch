...
public class InternalEngineTests extends ElasticsearchLuceneTestCase {
    ...
    public void testSegments() throws Exception {
        ...
        engine.create(new Engine.Create(null, newUid("3"), doc3, false));
        engine.refresh("test", false);
        ...
        engine.create(new Engine.Create(null, newUid("4"), doc4, false));
        engine.refresh("test", false);
    ...
    public void testSegmentsWithMergeFlag() throws Exception {
        ...
        Engine.Index index = new Engine.Index(null, newUid("1"), doc, false);
        engine.index(index);
        engine.flush(Engine.FlushType.COMMIT_TRANSLOG, false, false);
        assertThat(engine.segments(false).size(), equalTo(1));
        index = new Engine.Index(null, newUid("2"), doc, false);
        engine.index(index);
        engine.flush(Engine.FlushType.COMMIT_TRANSLOG, false, false);
        List<Segment> segments = engine.segments(false);
        assertThat(segments.size(), equalTo(2));
        ...
        index = new Engine.Index(null, newUid("3"), doc, false);
        engine.index(index);
        engine.flush(Engine.FlushType.COMMIT_TRANSLOG, false, false);
        segments = engine.segments(false);
        assertThat(segments.size(), equalTo(3));
        ...
        index = new Engine.Index(null, newUid("4"), doc, false);
        engine.index(index);
        engine.flush(Engine.FlushType.COMMIT_TRANSLOG, false, false);
    ...
    public void testSimpleOperations() throws Exception {
        ...
        engine.index(new Engine.Index(null, newUid("1"), doc, false));
    ...
    public void testRecoverWithOperationsBetweenPhase1AndPhase2() throws Exception {
        ...
        engine.create(new Engine.Create(null, newUid("1"), doc1, false));
        engine.flush(Engine.FlushType.COMMIT_TRANSLOG, false, false);
        ParsedDocument doc2 = testParsedDocument("2", "2", "test", null, -1, -1, testDocumentWithTextField(), Lucene.STANDARD_ANALYZER, B_2, false);
        engine.create(new Engine.Create(null, newUid("2"), doc2, false));
    ...
    public void testVersioningNewCreate() {
        ParsedDocument doc = testParsedDocument("1", "1", "test", null, -1, -1, testDocument(), Lucene.STANDARD_ANALYZER, B_1, false);
        Engine.Create create = new Engine.Create(null, newUid("1"), doc, false);
        engine.create(create);
        assertThat(create.version(), equalTo(1l));

        create = new Engine.Create(null, newUid("1"), doc, create.version(), create.versionType().versionTypeForReplicationAndRecovery(), REPLICA, 0, false);
    ...
    public void testExternalVersioningNewCreate() {
        ParsedDocument doc = testParsedDocument("1", "1", "test", null, -1, -1, testDocument(), Lucene.STANDARD_ANALYZER, B_1, false);
        Engine.Create create = new Engine.Create(null, newUid("1"), doc, 12, VersionType.EXTERNAL, Engine.Operation.Origin.PRIMARY, 0, false);
        engine.create(create);
    ...
    public void testVersioningNewIndex() {
        ...
        index = new Engine.Index(null, newUid("1"), doc, index.version(), index.versionType().versionTypeForReplicationAndRecovery(), REPLICA, 0, false);
    ...
    public void testExternalVersioningNewIndex() {
        ParsedDocument doc = testParsedDocument("1", "1", "test", null, -1, -1, testDocument(), Lucene.STANDARD_ANALYZER, B_1, false);
        Engine.Index index = new Engine.Index(null, newUid("1"), doc, 12, VersionType.EXTERNAL, PRIMARY, 0, false);
        engine.index(index);
    ...
    public void testVersioningIndexConflict() {
        ...
        index = new Engine.Index(null, newUid("1"), doc, false);
        engine.index(index);
        assertThat(index.version(), equalTo(2l));

        index = new Engine.Index(null, newUid("1"), doc, 1l, VersionType.INTERNAL, Engine.Operation.Origin.PRIMARY, 0, false);
        try {
            engine.index(index);
            fail();
        ...
        index = new Engine.Index(null, newUid("1"), doc, 3l, VersionType.INTERNAL, PRIMARY, 0, false);
        try {
            engine.index(index);
            fail();
        } catch (VersionConflictEngineException e) {
    ...
    public void testExternalVersioningIndexConflict() {
        ...
        index = new Engine.Index(null, newUid("1"), doc, 14, VersionType.EXTERNAL, PRIMARY, 0, false);
        engine.index(index);
        assertThat(index.version(), equalTo(14l));

        index = new Engine.Index(null, newUid("1"), doc, 13, VersionType.EXTERNAL, PRIMARY, 0, false);
        try {
            engine.index(index);
            fail();
        } catch (VersionConflictEngineException e) {
    ...
    public void testVersioningIndexConflictWithFlush() {
        ...
        index = new Engine.Index(null, newUid("1"), doc, 1l, VersionType.INTERNAL, PRIMARY, 0, false);
        try {
            engine.index(index);
            fail();
        } catch (VersionConflictEngineException e) {
    ...
    public void testVersioningDeleteConflict() {
        ...
        index = new Engine.Index(null, newUid("1"), doc, 2l, VersionType.INTERNAL, PRIMARY, 0, false);
        try {
            engine.index(index);
            fail();
        } catch (VersionConflictEngineException e) {
        ...
        Engine.Create create = new Engine.Create(null, newUid("1"), doc, 2l, VersionType.INTERNAL, PRIMARY, 0, false);
        try {
            engine.create(create);
        } catch (VersionConflictEngineException e) {
            // all is well
    ...
    public void testVersioningCreateExistsException() {
        ParsedDocument doc = testParsedDocument("1", "1", "test", null, -1, -1, testDocument(), Lucene.STANDARD_ANALYZER, B_1, false);
        Engine.Create create = new Engine.Create(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, PRIMARY, 0, false);
        engine.create(create);
        assertThat(create.version(), equalTo(1l));

        create = new Engine.Create(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, PRIMARY, 0, false);
        try {
            engine.create(create);
            fail();
    ...
    public void testVersioningReplicaConflict1() {
        ...
        index = new Engine.Index(null, newUid("1"), doc, index.version(), VersionType.INTERNAL.versionTypeForReplicationAndRecovery(), REPLICA, 0, false);
        replicaEngine.index(index);
        assertThat(index.version(), equalTo(2l));

        // now, the old one should not work
        index = new Engine.Index(null, newUid("1"), doc, 1l, VersionType.INTERNAL.versionTypeForReplicationAndRecovery(), REPLICA, 0, false);
        try {
            replicaEngine.index(index);
            fail();
        } catch (VersionConflictEngineException e) {
    ...
    public void testVersioningReplicaConflict2() {
        ...
        try {
            index = new Engine.Index(null, newUid("1"), doc, 2l, VersionType.INTERNAL.versionTypeForReplicationAndRecovery(), REPLICA, 0, false);
            replicaEngine.index(index);
            fail("excepted VersionConflictEngineException to be thrown");
        } catch (VersionConflictEngineException e) {
            // all is well
    ...
    public void testIndexWriterInfoStream() {
        ...
        try {
            ...
            engine.create(new Engine.Create(null, newUid("2"), doc, false));
            engine.flush(Engine.FlushType.COMMIT_TRANSLOG, false, false);
    ...
    public void testEnableGcDeletes() throws Exception {
        ...
        try {
            engine.index(new Engine.Index(null, newUid("1"), doc, 2, VersionType.EXTERNAL, Engine.Operation.Origin.PRIMARY, System.nanoTime(), false));
        ...
        try {
            engine.index(new Engine.Index(null, newUid("2"), doc, 2, VersionType.EXTERNAL, Engine.Operation.Origin.PRIMARY, System.nanoTime(), false));
            fail("did not hit expected exception");
        } catch (VersionConflictEngineException vcee) {
            // expected
        }
    ...
    @Test
    public void testIsRetryOnCreate() {

        ParsedDocument doc = testParsedDocument("1", "1", "test", null, -1, -1, testDocument(), Lucene.STANDARD_ANALYZER, B_1, false);
        Engine.Create index = new Engine.Create(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, PRIMARY, System.nanoTime(), false);
        engine.create(index);
        assertThat(index.version(), equalTo(1l));

        // if this is a retry of the previous request then version should stay the same and no exception
        index = new Engine.Create(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, PRIMARY, System.nanoTime(), true);
        engine.create(index);
        assertThat(index.version(), equalTo(1l));

        index = new Engine.Create(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, PRIMARY, System.nanoTime(), false);
        try {
            engine.create(index);
            fail("Expected a DocumentAlreadyExistsException");
        } catch (DocumentAlreadyExistsException e) {

        }

        // same should happen on replica
        index = new Engine.Create(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, REPLICA, System.nanoTime(), false);
        replicaEngine.create(index);
        assertThat(index.version(), equalTo(1l));

        // if this is a retry of the previous request then version should stay the same and no exception
        index = new Engine.Create(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, REPLICA, System.nanoTime(), true);
        replicaEngine.create(index);
        assertThat(index.version(), equalTo(1l));

        index = new Engine.Create(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, REPLICA, System.nanoTime(), false);
        replicaEngine.create(index);
        // on the primary we would expect this to throw an exception, but on the replica this is expected. Documents on primary and replica can have versions out of sync
        // see https://github.com/elasticsearch/elasticsearch/issues/7142
        assertThat(index.version(), equalTo(2l));

    }

    @Test
    public void testIsRetryOnIndex() {

        ParsedDocument doc = testParsedDocument("1", "1", "test", null, -1, -1, testDocument(), Lucene.STANDARD_ANALYZER, B_1, false);
        Engine.Index index = new Engine.Index(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, PRIMARY, System.nanoTime(), false);
        engine.index(index);
        assertThat(index.version(), equalTo(1l));

        // if this is a retry of the previous request then version should still increase
        index = new Engine.Index(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, PRIMARY, System.nanoTime(), true);
        engine.index(index);
        assertThat(index.version(), equalTo(2l));

        // same should happen on replica
        index = new Engine.Index(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, REPLICA, System.nanoTime(), false);
        replicaEngine.index(index);
        assertThat(index.version(), equalTo(1l));

        index = new Engine.Index(null, newUid("1"), doc, Versions.MATCH_ANY, VersionType.INTERNAL, REPLICA, System.nanoTime(), true);
        replicaEngine.index(index);
        assertThat(index.version(), equalTo(2l));
