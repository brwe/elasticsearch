 ...
 *
 */
public class SignificantTermsMissingAggsTests extends ElasticsearchIntegrationTest {

    public String randomExecutionHint() {
        return randomBoolean() ? null : randomFrom(SignificantTermsAggregatorFactory.ExecutionMode.values()).toString();
    }
    @Test
    public void missingAggs() throws Exception {
        String termtype = "string";
        if (randomBoolean()) {
            termtype = "long";
        }
        assertAcked(prepareCreate("missingtermsindex").setSettings(SETTING_NUMBER_OF_SHARDS, 1, SETTING_NUMBER_OF_REPLICAS, 0).addMapping("testtype", "{\"properties\":{\"text\": {\"type\": \"" + termtype + "\"}}}"));
        ensureYellow("missingtermsindex");
        ...
        refresh();
        SearchResponse response = client().prepareSearch("missingtermsindex")
                .addAggregation(
    ...
    private void indexTermsDocs(String term, int numInClass, int numNotInClass) {
        ...
        for (int i = 0; i < numInClass; i++) {
            client().prepareIndex("missingtermsindex", "testtype").setSource(sourceClass).execute().actionGet();
        }
        for (int i = 0; i < numNotInClass; i++) {
            client().prepareIndex("missingtermsindex", "testtype").setSource(sourceNotClass).execute().actionGet();
        }
    }
