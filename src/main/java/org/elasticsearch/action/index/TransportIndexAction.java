    ...
    protected PrimaryResponse<IndexResponse, IndexRequest> shardOperationOnPrimary(ClusterState clusterState, PrimaryOperationRequest shardRequest) {
        ...
        if (request.opType() == IndexRequest.OpType.INDEX) {
            ...
            if (index.parsedDoc().mappingsModified()) {
                try {
                    logger.debug("dev-issue-195 TransportIndexAction: in shardOperationOnPrimary 196 call updateMappingOnMaster with mapping [{}] on index [{}], uuid is [{}], request index [{}]", index.docMapper().mappingSource().string(), indexService.index(), indexService.indexUUID(), request.index());
                } catch (IOException e) {
            ...
            if (create.parsedDoc().mappingsModified()) {
                try {
                    logger.debug("dev-issue-195 TransportIndexAction: in shardOperationOnPrimary 211 call updateMappingOnMaster with mapping [{}] on index [{}], uuid is [{}], request index [{}]", create.docMapper().mappingSource().string(), indexService.index(), indexService.indexUUID(), request.index());
                } catch (IOException e) {
