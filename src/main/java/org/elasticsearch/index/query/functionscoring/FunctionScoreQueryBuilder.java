 ...
 * score.
 */
public class FunctionScoreQueryBuilder extends BaseQueryBuilder implements
        BoostableQueryBuilder<FunctionScoreQueryBuilder> {
    ...
    protected void doXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        if(filters.size()==1 && filters.get(0)==null) {
            if (scoreMode != null) {
                throw new ElasticSearchException("Cannot set scoreMode if only one score function is given.");
            }
            if (maxBoost != null) {
                throw new ElasticSearchException("Cannot set maxBoost if only one score function is given.");
            }
