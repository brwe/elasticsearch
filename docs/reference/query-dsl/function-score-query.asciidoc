...
format. I addition, each function must contain a parameter `var_name` which can then be used as a parameter in the script. For example,
the following query will multiply the result of the first two functions and then subtract the result of the third:
...
<2> The script to use when the individual scores are combined
<3> score_mode "script" must be set in this case


TODO: What to do when a function does not match? Decide and then add doc.
